    ...
    */
    ...
    selectQuery: function(tableName, options, Model) {
      ...
      var table               = null
        ...
        , subQuery            = options.subQuery !== undefined ? options.subQuery : limit && (options.hasIncludeWhere || options.hasIncludeRequired || options.hasMultiAssociation)
        , subQueryItems       = []
          ...
          if (through) {
            ...
            if (include.where) {
              ...
              if (subQuery && association.isMultiAssociation) {
                ...
                // Creating the as-is where for the subQuery, checks that the required association exists
                if (include.required) { //  && Object.keys(Utils._.omit(options.where, 'deleted_at')).length > 0
